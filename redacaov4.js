// ================================================================
// PAINEL REDAÇÃO PARANÁ V3 - COM SALVAMENTO NA PLATAFORMA
// Senha: hack123
// (c) 2026 - Clique no botão "SALVAR RASCUNHO" da plataforma
// ================================================================
...
(function() {
    ...
    const CONFIG = {
        password: 'hack123',
        panelTitle: '⌨️ Redação PR',
        panelSubtitle: 'autotyper v3',
        speeds: {
            normal: 100,
            rapido: 25,
            extremo: 3
    ...
    // ---------- ESTILOS CSS (mesmos do anterior) ----------
    ...
    style.textContent = `
        ...
        #saveBtn { background: #ffcc00; color: #0b0b15; }
        #saveBtn:hover { background: #e6b800; transform: scale(1.02); }
        ...
        .status-bar #statusText.saved { color: #ffcc00; }
    ...
    // ========== FUNÇÃO PARA SALVAR NA PLATAFORMA ==========
    function salvarRedacao() {
        // Tenta localizar o botão "SALVAR RASCUNHO"
        // O elemento que você passou é um <p> com esse texto.
        // Ele provavelmente está dentro de um botão ou elemento clicável.
        // Vamos procurar por qualquer elemento que contenha esse texto e seja clicável.
        const elementos = document.querySelectorAll('*');
        let botaoSalvar = null;

        for (let el of elementos) {
            // Verifica se o texto interno contém "SALVAR RASCUNHO" (case insensitive)
            if (el.innerText && el.innerText.trim().toUpperCase() === 'SALVAR RASCUNHO') {
                // Se o elemento for um <p>, tenta subir para o pai que seja clicável (button, div com role button, etc.)
                let pai = el;
                while (pai && pai !== document.body) {
                    if (pai.tagName === 'BUTTON' || pai.getAttribute('role') === 'button' || pai.style.cursor === 'pointer' || pai.onclick) {
                        botaoSalvar = pai;
                        break;
                    }
                    pai = pai.parentElement;
                }
                // Se não achou um pai clicável, usa o próprio elemento (se for clicável)
                if (!botaoSalvar && (el.tagName === 'BUTTON' || el.getAttribute('role') === 'button' || el.style.cursor === 'pointer')) {
                    botaoSalvar = el;
                }
                // Se ainda não achou, usa o pai imediato (que pode ser um div clicável)
                if (!botaoSalvar && el.parentElement) {
                    botaoSalvar = el.parentElement;
                }
                break;
            }
        }

        // Se não achou, tenta seletores alternativos
        if (!botaoSalvar) {
            const seletores = [
                'button:contains("SALVAR RASCUNHO")',
                'button:contains("Salvar Rascunho")',
                'button:contains("salvar rascunho")',
                '[data-cy="salvar-rascunho"]',
            ...
            for (let sel of seletores) {
                ...
                if (sel.includes(':contains')) {
                    const texto = sel.match(/:contains\("(.+?)"\)/)[1];
                    const botoes = document.querySelectorAll('button, input[type="button"], input[type="submit"], div[role="button"], a');
                    for (let btn of botoes) {
                        if (btn.innerText && btn.innerText.toLowerCase().includes(texto.toLowerCase())) {
                            botaoSalvar = btn;
                            break;
                        }
                    }
                } else {
                    botaoSalvar = document.querySelector(sel);
                }
                if (botaoSalvar) break;
            }
        }
        ...
        if (botaoSalvar) {
            // Clica no botão
            botaoSalvar.click();
            setStatus('✅ Rascunho salvo na plataforma!', 'saved');
            console.log('✅ Botão "SALVAR RASCUNHO" clicado com sucesso!', botaoSalvar);
        } else {
            setStatus('❌ Botão "SALVAR RASCUNHO" não encontrado. Tente manualmente.', 'stopped');
            console.warn('❌ Não foi possível encontrar o botão "SALVAR RASCUNHO" na página.');
            // Tenta uma última abordagem: procura por qualquer elemento com texto "salvar" e tenta clicar
            const all = document.querySelectorAll('button, input, a, div');
            for (let el of all) {
                const txt = el.innerText || el.value || '';
                if (txt.toLowerCase().includes('salvar') && !txt.toLowerCase().includes('corrigir')) {
                    el.click();
                    setStatus('⚠️ Clicou em um botão "Salvar" alternativo.', 'saved');
                    console.log('⚠️ Clicou em elemento alternativo:', el);
                    return;
                }
            }
        }
    ...
    startBtn.addEventListener('click', function() {
        ...
        function typeNextChar() {
            ...
            if (index > totalLength) {
                ...
                // ========== SALVAMENTO AUTOMÁTICO NA PLATAFORMA ==========
                salvarRedacao();
                return;
            }
    ...
    pwdInput.focus();
    console.log('🟢 Painel Redação Paraná V3 carregado. Senha: hack123');
})();
